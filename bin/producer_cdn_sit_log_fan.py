...
def generate_normal_log(filename='data/test.log', channel='zhongzh.test.com', topic='ctYun', flag=0):
    ...
    for timestamp in range(now - 8 * 3600, now, 100):
        cnt += 1
        ...
        flow = int(flow_list[interal] * 125000)
        ...
        for i in range(0, 10):
            log_sample = {"serverIp": "14.215.109.226",
                          ...
                          "channel": channel,
                          "url": "http://" + channel + url + random_int('1,3') + '/',
...
def generate_fan_log(filename='data/test.log', channel='zhongzh.test.com', topic='ctYun', flag=0):
    fd = open(filename, 'w+')
    ...
    status_list = {'HIT': 90, 'MISS': 10}
    # url_type = ['video', 'image', 'txt']
    # type = random.choice(url_type)
    url_type = {'video': 90, 'image': 5, 'txt': 5}
    type = random_weight(url_type)
    url = '/' + random_string(32) + '/' + random_string(8) + '/' + type + '/tos/cn/tos-ve-cn-' + random_int(
        '10, 10000') + '/'
    day_time = int(time.mktime(datetime.now().date().timetuple()))
    for timestamp in range(day_time, now, 300):
        cnt += 1
        clientIp = random.choice(clientIp_list)
        httpcode = random_weight(httpcode_list)
        referer2 = random.choice(get_referer())
        ...
        temp = timestamp - day_time
        interal = int(temp / 300)
        flow = int(flow_list[interal] * 125000 * 100)
        for i in range(0, 10):
            log_sample = {"serverIp": "14.215.109.226",
                          "timestamp": str(timestamp),
                          "respondTime": 1,
...
if __name__ == '__main__':
    ...
    # domain_list = ['cmq004.test004.cn', 'cmq002.ksedu.cn', 'cmq003.ksedu.cn']
    zhouqi_domain_list = ['www.test007.cn', 'zq28.spdb.pay.jclinx.com', 'a.ss.baidu.com', 'b.ss.baidu.com',
                          'cmq004.test004.cn']
    fan_domain_list = ['a.cmqtest07.com', 'b.cmqtest07.com', 'c.cmqtest07.com', 'cmq004.test004.cn', 'cmq002.ksedu.cn']
    for domain in zhouqi_domain_list:
        filepath = "C:\python37\Scripts\logsystem\data/" + domain + "_" + timestamp + ".log"
        generate_normal_log(filepath, domain, tp, 1)
    for item in fan_domain_list:
        filepath = "C:\python37\Scripts\logsystem\data/" + item + "_" + timestamp + ".log"
        generate_fan_log(filepath, item, tp, 1)
